...
// https://github.com/jogolden/testdev/blob/master/xkelib/xam/_xamext.h#L379
void XamLoaderLaunchTitleEx_entry(lpstring_t launch_path,
                                            lpstring_t mount_path,
                                            lpstring_t cmd_line,
                                            dword_t flags) {
  auto xam = kernel_state()->GetKernelModule<XamModule>("xam.xex");
  ...
  if (!launch_path) {
    // assume we're returning to the dashboard
...
void XamLoaderGetMediaInfo_entry(lpdword_t media_type_out, dword_t unk2) {
  // 0 - No Disc
  // 1 - Game Disc
  // 3 - HD DVD
  // 5 - DVD
  ...
  if (kernel_state()->file_system()->ResolvePath("\\Device\\Cdrom0")) {
    xe::store_and_swap<uint32_t>(media_type_out, 1);
  } else {
    xe::store_and_swap<uint32_t>(media_type_out, 0);
  }
}
