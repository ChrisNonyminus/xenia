...
bool EmulatorWindow::Initialize() {
  ...
  auto file_menu = MenuItem::Create(MenuItem::Type::kPopup, "&File");
  {
    ...
    file_menu->AddChild(
        MenuItem::Create(MenuItem::Type::kString, "&Mount Disc...", "Ctrl+O",
                         std::bind(&EmulatorWindow::MountDiscImage, this)));
#ifdef DEBUG
    file_menu->AddChild(
...
void EmulatorWindow::MountDiscImage() {
  std::filesystem::path path;
  ...
  if (!path.empty()) {
    // Normalize the path and make absolute.
    auto abs_path = std::filesystem::absolute(path);
    auto result = emulator_->MountDiscImage(abs_path);
    if (XFAILED(result)) {
      // TODO: Display a message box.
      XELOGE("Failed to mount disc: {:08X}", result);
    }
  }
}
